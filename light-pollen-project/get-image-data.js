  ...
  for(let i = 0; i < rowAmount; i++){
    ...
    for(let k = 0; k < 4; k++){
      ...
      if(sizeNum >= fileNames.length){
        ...
        currentImageName ="images/"+ fileNames[sizeNum];
        if (dataName == "art_portfolio_icon_images.txt" || dataName == "art_portfolio_emote_images.txt"){

          currentRow.innerHTML += "<button class='clear-button' onClick='viewImage(this.value);' value='" + currentImageName + "' > <img class='small-gallery-img' src='" + currentImageName + "'/></button>";

        } else if (dataName == "art_portfolio_pngtuber_images.txt"){
          currentRow.innerHTML += "<button class='clear-button' onClick='viewImage(this.value);' value='" + currentImageName + "' > <img class='long-gallery-img' src='" + currentImageName + "'/></button>";
        } else {
          currentRow.innerHTML += "<button class='clear-button' onClick='viewImage(this.value);' value='" + currentImageName + "' > <img class='gallery-img' src='" + currentImageName + "'/></button>";
        }
      }
    }
